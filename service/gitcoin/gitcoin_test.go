...
func TestGitcoinPassport(t *testing.T) {
	...
	// create the provider
	provider := &GitcoinPassport{APIEndpoint: originalEndpoint}
	c.Assert(provider.Init(), qt.IsNil)
	// start the first download
	emptyBalances, err := provider.HoldersBalances(context.TODO(), nil, 0)
	c.Assert(err, qt.IsNil)
	c.Assert(len(emptyBalances), qt.Equals, 0)
	// wait for the download to finish
	time.Sleep(2 * time.Second)
	// check the balances
	holders, err := provider.HoldersBalances(context.TODO(), nil, 0)
	c.Assert(err, qt.IsNil)
	...
	sameBalances, err := provider.HoldersBalances(context.TODO(), nil, 0)
	c.Assert(err, qt.IsNil)

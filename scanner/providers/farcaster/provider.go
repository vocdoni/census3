...
func (p *FarcasterProvider) Close() error {
	p.cancelScanner()
	log.Info("closing farcaster provider 1")
	if err := p.db.CloseDB(); err != nil {
		return fmt.Errorf("cannot close web3 client %s", err.Error())
	}
	log.Info("closing farcaster provider 2")
	return nil
}

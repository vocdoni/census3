...
func (q *Queries) TokenHolderByTokenIDAndHolderIDAndChainIDAndExternalID(ctx context.Context, arg TokenHolderByTokenIDAndHolderIDAndChainIDAndExternalIDParams) (TokenHolder, error) {
...
SELECT tokens.id, tokens.name, tokens.symbol, tokens.decimals, tokens.total_supply, tokens.creation_block, tokens.type_id, tokens.synced, tokens.tags, tokens.chain_id, tokens.chain_address, tokens.external_id, tokens.default_strategy, tokens.icon_uri, tokens.created_at, tokens.last_block, tokens.analysed_transfers
FROM tokens

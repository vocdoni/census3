...
func TestWeb3Init(t *testing.T) {
	...
	c.Assert(new(Web3).Init(ctx, "", MonkeysAddress, CONTRACT_TYPE_ERC20, nil), qt.IsNotNil)
	c.Assert(new(Web3).Init(ctx, web3URI, MonkeysAddress, CONTRACT_TYPE_ERC20, nil), qt.IsNil)
}
...
func TestTokenName(t *testing.T) {
	...
	w := new(Web3)
	c.Assert(w.Init(ctx, "https://google.com", MonkeysAddress, CONTRACT_TYPE_ERC20, nil), qt.IsNil)
	_, err := w.TokenName()
	c.Assert(err, qt.IsNotNil)

	c.Assert(w.Init(ctx, web3URI, common.HexToAddress("0x0"), CONTRACT_TYPE_ERC20, nil), qt.IsNil)
	_, err = w.TokenName()
	c.Assert(err, qt.IsNotNil)

	c.Assert(w.Init(ctx, web3URI, MonkeysAddress, CONTRACT_TYPE_ERC20, nil), qt.IsNil)
...
func TestUpdateTokenHolders(t *testing.T) {
	...
	th := new(TokenHolders)
	th = th.Init(MonkeysAddress, CONTRACT_TYPE_ERC20, MonkeysCreationBlock, 5, nil)

	w3 := Web3{}
	ctx, cancel := context.WithTimeout(context.Background(), 3000*time.Second)
	defer cancel()

	c.Assert(w3.Init(ctx, web3URI, th.Address(), th.Type(), nil), qt.IsNil)
...
func Test_commitTokenHolders(t *testing.T) {
	...
	th := new(TokenHolders).Init(MonkeysAddress, CONTRACT_TYPE_ERC20, MonkeysCreationBlock, 5, nil)
...
func TestCreationBlock(t *testing.T) {
	...
	w := new(Web3)
	c.Assert(w.Init(ctx, web3URI, common.HexToAddress(""), CONTRACT_TYPE_ERC20, nil), qt.IsNil)
